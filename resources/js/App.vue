    ...
    <div class="bg-gray-100 flex" v-if="authStore.user && authStore.user.hasOwnProperty('id')">
        <aside class="relative bg-theme-600 h-screen w-64 hidden sm:block shadow-xl">
            <div class="p-6 border-b border-theme-600">
                <router-link class="text-white text-3xl font-semibold uppercase hover:text-gray-300" to="/panel/listing/users">
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            mainMenu: [
                {
                    name: trans('global.pages.jobs'),
                    icon: 'check-square',
                    showDesktop: true,
                    showMobile: true,
                    requiresAbility: false,
                    to: '/panel/listing/jobs',
                },
                {
